...
if __name__=='__main__':
    ...
    if stage3==1:
        ...
        for i in range(len(image_paths)):
            if type(des_list[i][1])==np.ndarray:
                ...
                words, distance = vq(des_list[2][1],voc) # give errordata a uniform representative.2 is optional.
            for w in words:
                im_features[i][w] += 1
            if i%1000==0:
                print time.ctime(),i
        # saving the total feature..
        np.save('feat_{}_{}.npy'.format(s_,e_),im_features)

    else:
        print 'directly load the local transformed BOW feature.'
        im_features=np.load('feat_{}_{}.npy'.format(s_,e_))
    ...
    if stage4==1:
        ...
        lines.sort()
        lines=lines[s_:e_]
        video_nframe_dic={_.split(' ')[0].split('/')[-1]:int(_.split(' ')[1])-1 for _ in lines}
        ...
        joblib.dump(video_feat_dic,'video_bow_feat_{}_{}.pkl'.format(s_,e_))
